...
class SearchPlacesScreen2 extends StatefulWidget {
  const SearchPlacesScreen2({super.key});

  @override
  State<SearchPlacesScreen2> createState() => _SearchPlacesScreen2State();
}
...
class _SearchPlacesScreen2State extends State<SearchPlacesScreen2> {
  ...
  findPlaceAutoCompleteSearch(String inputText) async {
    if(inputText.length > 1){
      ...
      var responseAutoCompleteSearch2 = await RequestAssistant.receiveRequest(urlAutoCompleteSearch);

      if(responseAutoCompleteSearch2 == "Error occured. Failed . No Response"){
        return;
      }

      if(responseAutoCompleteSearch2["status"] == "OK"){
        var placePredictions = responseAutoCompleteSearch2["predictions"];
